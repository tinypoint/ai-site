...
| 步骤 | 功能说明 | 输出标准 |
|------|----------|----------|
| 1    | 创建站点蓝图，指导后续步骤 | 包含页面功能、组件、布局、样式、属性、接口请求、数据绑定关系，事件交互的结构化文档 |
| 2    | 定义接口请求清单 | 完整接口列表（名称/URL/参数/响应结构） |
| 3    | 组件架构设计 | 带唯一ID的组件树（含类型/父子关系/布局样式/属性） |
| 4    | 生成Mock数据 | 符合接口定义的JSON数据集 |
| 5    | 事件流设计 | 事件-动作映射关系表 |
| 6    | 数据关联配置 | 使用{{expression}}语法的绑定关系表 |
| 7    | 生成最终Schema | 符合AI SITE Schema规范的JSON文件 |
